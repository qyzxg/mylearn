...
while True:
    ...
    cur = conn.cursor()
    status = random.choice([200, 301, 302, 404, 502])
    confirmed = 1
    confirmed_on = fake.date_time_between(start_date="-28d", end_date="now", tzinfo=None)
    post_total = fake.random_int(min=0, max=9999)
    wx_num = random.random()
    zfb_num = random.random()
    url = fake.url()
    ip_addr = fake.ipv4(network=False)
    cur = conn.cursor()
    ins = (
        "INSERT ignore `blog`.`loginfo` (`id`, `ip`, `time_r`, `status_code`, `length`, `url`, `req_time`, `res_time`, `time_stamp`) VALUES ('{}', '{}', '{}', '{}', '{}','{}', '{}', '{}', '{}');".format(
            id, ip_addr, confirmed_on, status, post_total, str(url), wx_num, zfb_num, int(time.time())))
    cur.execute(ins)
    conn.commit()
    cur.close()
    conn.close()
